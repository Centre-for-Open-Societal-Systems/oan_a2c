...
def _get_subscription_secret():
	...
	if not secret:
		...
		frappe.log_error("websub_secret missing from site_config.json", "WebSub Subscriber Misconfigured")
	return secret
...
def callback(**kwargs):
	...
	if req.method == "GET":
		...
		frappe.logger().info(f"🤝 WebSub intent verification: mode={mode!r} topic={topic!r}")
		if not challenge:
