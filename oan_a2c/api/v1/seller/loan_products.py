...
def set_product_status(**kwargs):
	...
	# Idempotent short-circuit: re-sending the status a product already has is a
	# success, not a conflict. Checked before the archive gate below so re-sending
	# "Archived" on an already-Archived product is never blocked by it.
