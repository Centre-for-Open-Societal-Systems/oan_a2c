...
def _user(email, role):
	if not frappe.db.exists("User", email):
		frappe.get_doc(
			{"doctype": "User", "email": email, "first_name": email.split("@")[0], "roles": [{"role": role}]}
	...
	return frappe.get_doc(
		{
			...
			"bank_code": code,
			"entity_type": "Bank",
			"registered_email": f"reg-{h}@ex.com",
...
class TestSecurityRegression(unittest.TestCase):
	...
	def test_A3_bank_admin_cannot_delete_arbitrary_file(self):
		...
		victim = frappe.get_doc(
			{"doctype": "File", "file_name": f"victim-{h}.txt", "content": "secret-kyc", "is_private": 1}
		).insert(ignore_permissions=True)
	...
	def test_A3_bank_admin_can_change_own_logo(self):
		...
		self.assertEqual(frappe.db.get_value("A2C Participating Bank", bank.name, "logo"), own.file_url)
