...
Companion to `A2C_API_REST_Routing_Map.md` and `openapi_v1.public.yaml`. Covers deploying the 94-route REST facade behind Kong, wiring authentication, and setting throttling — as a phased, reviewable rollout rather than a single cutover.
...
- `kong.yml` — generated declarative config (94 routes, 1 service, 4 example consumers). Placeholders to fill in before applying: the upstream URL, the JWT issuer's RSA public key, webhook partner API keys, and the `ip-restriction` CIDR blocks.
