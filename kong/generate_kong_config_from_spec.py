    ...
    for path, methods in spec["paths"].items():
        for method, op in methods.items():
            ...
            routes.append({
                ...
                "domain": domain, "tag": tag, "operation_id": op.get("operationId"),
            })
    return routes
...
def build_config(routes):
    service = {
        ...
        "plugins": [
            ...
            },
            {
                ...
                "config": {"allowed_payload_size": 20},  # MB, global safety net (uploads tier overrides tighter)
            },
            {
                "name": "correlation-id",
    ...
    for r in routes:
        ...
        if auth == "bearer":
            ...
            route["plugins"].append({"name": "key-auth", "config": {"key_names": ["apikey"], "hide_credentials": True}})
            route["plugins"].append({"name": "ip-restriction", "config": {"allow": ["203.0.113.0/24"]}})  # placeholder CIDR
...
def main():
    ...
    print(f"wrote kong.yml: {len(routes)} routes from {len(spec['paths'])} paths "
          f"({len(spec['tags'])} domains) -- methods: {by_method}", file=sys.stderr)
